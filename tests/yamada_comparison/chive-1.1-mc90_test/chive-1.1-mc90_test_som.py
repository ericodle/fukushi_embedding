...
som_cluster_labels = som.fit_predict(embedding_matrix)
...
for i, perm in enumerate(label_permutations):
    ...
    cluster_labels_permutations[var_name] = [perm[label] for label in som_cluster_labels]
...
for var_name, perm_labels in cluster_labels_permutations.items():
    ...
    som_accuracies[var_name] = accuracy
...
best_result_key = max(som_accuracies, key=som_accuracies.get)
...
best_result_accuracy = som_accuracies[best_result_key]
...
som_truncated_accuracy = "{:.3f}".format(best_result_accuracy)

# Generate confusion matrix plot for the best result with custom display labels
som_conf_matrix = confusion_matrix(true_labels, best_result_labels)
disp = ConfusionMatrixDisplay(som_conf_matrix, display_labels=true_labels_display_labels)
disp.plot(cmap='Blues', values_format='d', xticks_rotation='horizontal', ax=plt.gca())
plt.title(f'Confusion Matrix | Acc: {som_truncated_accuracy}')
plt.tight_layout()

# Save the plots to the test_directory
plt.savefig(test_directory+'som_accuracy.png')
