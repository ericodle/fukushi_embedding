...
# Load
######################################################################
vectors = Magnitude("/home/eo/Desktop/fukushi_embedding/models/chive-1.1-mc90.magnitude")
test_directory = "/home/eo/Desktop/fukushi_embedding/tests/yamada_comparison/chive-1.1-mc90_test/"
...
hierarchical_cluster_labels = hierarchical_clustering.fit_predict(embedding_matrix)
...
hierarchical_label_permutations = list(permutations(range(num_clusters)))

# Create variables for each permutation
hierarchical_cluster_labels_permutations = {}

for i, perm in enumerate(hierarchical_label_permutations):
    ...
    hierarchical_cluster_labels_permutations[var_name] = [perm[label] for label in hierarchical_cluster_labels]
...
for var_name, perm_labels in hierarchical_cluster_labels_permutations.items():
    # Calculate accuracy for the current permutation
    accuracy = accuracy_score(true_labels, perm_labels)
    
    # Store the accuracy in the dictionary
    hierarchical_accuracies[var_name] = accuracy
...
hierarchical_best_result_key = max(hierarchical_accuracies, key=hierarchical_accuracies.get)

# Get the cluster labels for the best result
hierarchical_best_result_labels = hierarchical_cluster_labels_permutations[hierarchical_best_result_key]

# Print the best result accuracy
hierarchical_best_result_accuracy = hierarchical_accuracies[hierarchical_best_result_key]
...
hierarchical_truncated_accuracy = "{:.3f}".format(hierarchical_best_result_accuracy)

# Generate confusion matrix plot for the best result with custom display labels
hierarchical_conf_matrix = confusion_matrix(true_labels, hierarchical_best_result_labels)
disp = ConfusionMatrixDisplay(hierarchical_conf_matrix, display_labels=true_labels_display_labels)
disp.plot(cmap='Blues', values_format='d', xticks_rotation='horizontal', ax=plt.gca())
plt.title(f'Confusion Matrix | Acc: {hierarchical_truncated_accuracy}')
plt.tight_layout()

# Save the plots to the test_directory
plt.savefig(test_directory+'hierarchical_accuracy.png')
