...
spectral_cluster_labels = spectral_clustering.fit_predict(embedding_matrix)
...
# Generate all permutations of cluster labels
spectral_label_permutations = list(permutations(range(num_clusters)))

# Create variables for each permutation
spectral_cluster_labels_permutations = {}

for i, perm in enumerate(spectral_label_permutations):
    # Create a new variable for each permutation, e.g., cluster_labels_0, cluster_labels_1, ...
    var_name = f'cluster_labels_{i}'
    
    # Swap the cluster labels based on the permutation
    spectral_cluster_labels_permutations[var_name] = [perm[label] for label in spectral_cluster_labels]
...
for var_name, perm_labels in spectral_cluster_labels_permutations.items():
    # Calculate accuracy for the current permutation
    accuracy = accuracy_score(true_labels, perm_labels)
    
    # Store the accuracy in the dictionary
    spectral_accuracies[var_name] = accuracy
...
spectral_best_result_key = max(spectral_accuracies, key=spectral_accuracies.get)

# Get the cluster labels for the best result
spectral_best_result_labels = spectral_cluster_labels_permutations[spectral_best_result_key]

# Print the best result accuracy
spectral_best_result_accuracy = spectral_accuracies[spectral_best_result_key]
...
spectral_truncated_accuracy = "{:.3f}".format(spectral_best_result_accuracy)

# Generate confusion matrix plot for the best result with custom display labels
spectral_conf_matrix = confusion_matrix(true_labels, spectral_best_result_labels)
disp = ConfusionMatrixDisplay(spectral_conf_matrix, display_labels=true_labels_display_labels)
disp.plot(cmap='Blues', values_format='d', xticks_rotation='horizontal', ax=plt.gca())
plt.title(f'Confusion Matrix | Acc: {spectral_truncated_accuracy}')
plt.tight_layout()

# Save the plots to the test_directory
plt.savefig(test_directory+'spectral_accuracy.png')
